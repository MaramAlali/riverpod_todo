...
class DBHelper {
  static Future<void> createTables(sql.Database database) async {
    await database.execute("CREATE TABLE todos ("
        ...
        "title STRING ,description TEXT,date STRING ,startTime STRING,"
        "endTime STRING,remind INTEGER ,repeat INTEGER,"
        "isCompleted INTEGER ) ");
    await database.execute("CREATE TABLE user("
        "id INTEGER PRIMARY KEY AUTOINCREMENT DEFAULT 0,isVerified INTEGER  "
        ")");
  }

  static Future<sql.Database> db() async {
    return sql.openDatabase('todoDB', version: 1,
  ...
  static Future<void> deleteItem(int id) async {
    ...
    try {
      ...
      // debugPrint("unable to delete this item");
    }
  }
}
